		...
		SubShader
			{
				...
				half4 LitPassFragment(Varyings input) : SV_Target
				{
					...
					float fresnel_d = 1 - dot(normalWS, viewDirectionWS);
					//fresnel_d *= _FresnelStrength;
					//float4 fresnel_col = _FresnelColor * fresnel_d;
					float3 fresnel_col = _EmissionColor * fresnel_d;
					fresnel_col *= _Smoothness;

					//float4 fresnel_col = float4(1, 0, 0 ,0) * fresnel_d;

					color = MixFog(color, fogFactor) + fresnel_col;
